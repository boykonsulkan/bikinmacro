...
export default function GenerateForm({ 
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const html = await codeToHtml(data.vba_code, {
        lang: 'vba',
        theme: 'github-dark'
      })
      setOutputHtml(html)
      
      router.refresh() // To update server-side credit count
    } catch (err: any) {
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      <div className="flex flex-col gap-4">
        <div className="bg-card border border-border p-6 rounded-2xl">
          ...
          <label className="block text-sm font-medium mt-4 mb-2 text-foreground">
            Kategori
          </label>
          <select
          ...
          {error && <div className="mt-4 p-3 bg-red-900/20 border border-red-900 text-red-400 rounded-lg text-sm">{error}</div>}

          <button
            onClick={handleGenerate}
            disabled={isLoading || !prompt.trim() || !hasCredits}
      ...
      <div className="flex flex-col gap-4">
          ...
          <div className="bg-card border border-border rounded-2xl overflow-hidden flex flex-col h-full">
            <div className="bg-background border-b border-border px-4 py-3 flex justify-between items-center">
              ...
              <div className="flex gap-2">
                <button onClick={copyToClipboard} className="p-2 hover:bg-card rounded-md text-muted hover:text-foreground transition-colors flex items-center gap-1 text-xs">
                  {isCopied ? <Check size={14} className="text-green-500" /> : <Copy size={14} />}
                  {isCopied ? 'Tersalin' : 'Salin'}
                </button>
                <button onClick={downloadFile} className="p-2 hover:bg-card rounded-md text-muted hover:text-foreground transition-colors flex items-center gap-1 text-xs">
            ...
            <div className="border-t border-border p-4 bg-background flex justify-between">
               <button onClick={() => handleGenerate()} className="flex items-center gap-2 text-sm text-muted hover:text-foreground">
                 <RefreshCw size={16} /> Regenerate
               </button>
               <button onClick={resetForm} className="flex items-center gap-2 text-sm text-primary hover:text-primary-hover font-medium">
        ...
        {outputRaw && (
          <div className="bg-card border border-border p-6 rounded-2xl mt-4">
